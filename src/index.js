...
function populateProductData() {
  for (let i = 0; i < products.length; i++) {
    ...
    for (let j = 1; j < products[i].url.length; j++) {
      const thumbnailsCropped = transformImageUrl(
        products[i].url[j],
        2400,
        2400
      );
    ...
    document.getElementById(
      `discount-badge-${i}`
    ).innerText = `-${products[i].discount}%`;
    document.getElementById(
      `sale-price-${i}`
    ).innerText = `LE ${calculateDiscount(
      products[i].price,
      products[i].discount
    )}.00`;
    document.getElementById(
      `original-price-${i}`
    ).innerText = `LE ${products[i].price}.00`;
  }
}
...
function addDataToPopup(id) {
  ...
  document
    .getElementById("close-btn-popUp")
