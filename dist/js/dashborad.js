...
async function getProductsFromDB(collectionsNames) {
  for (let i = 0; i < collectionsNames.length; i++) {
    await getDocs(collection(db, collectionsNames[i])).then((snapshot) => {
...
function addFilteringEventListeners() {
  ...
  filterSelect.addEventListener("change", (e) => {
    filterProducts = filterdProductsByCategory(
      originalProducts,
      e.target.value
    );
  ...
  if (productsToRender.length === 0) {
    ...
    for (let i = 0; i < productsToRender.length; i++) {
      ...
      row.innerHTML = `
                ...
                <td>
                    <button class="btn btn-sm btn-outline-dark edit-btn" data-id="${
                      productsToRender[i].id
                    }">
    ...
    let imagesOfTable = document.querySelectorAll(
      "#product-table-body tr td img"
    );
...
function addingEventListners() {
  ...
  document.addEventListener("click", (e) => {
    if (
      ...
      e.target.id !== "target-div-To-Show-photo"
    ) {
      targetDiv.style.cssText = "";
    }
  });
...
async function uploadImage(fileInput, currentSpanId) {
  ...
  try {
    const response = await fetch(
      "https://api.cloudinary.com/v1_1/denqwkum6/image/upload",
      {
        method: "POST",
        body: formData,
      }
...
function initializeSaveProduct() {
  ...
  saveProductBtn.addEventListener("click", function (e) {
    ...
    const sizes = [];
    document
      .querySelectorAll("#sizesContainer .size-item-m")
      .forEach((item) => {
        const value = item
          .querySelector('input[name="sizeValue[]"]')
          .value.trim();
        if (value) {
          sizes.push(value);
        }
...
function openEditModal(productId) {
  ...
  if (product) {
    ...
    document.getElementById("edit-product-id").value = product.id;
    document.getElementById("edit-product-collection").value =
...
function displayCurrentImages(imageUrls) {
  ...
  currentImagesContainer
    .querySelectorAll(".delete-image-btn")
    .forEach((button) => {
...
document
  .getElementById("editAddImageBtn")
  .addEventListener("click", function () {
...
document.addEventListener("click", function (e) {
  if (
    e.target &&
    e.target.classList.contains("remove-btn-m-img") &&
    e.target.id.includes("edit-remove")
  ) {
    const parentDiv = e.target.parentElement;
    const input = parentDiv.querySelector("input[type='file']");
    const inputId = input ? input.id : undefined;
...
function initializeUpdateProduct() {
  ...
  updateProductBtn.addEventListener("click", function (e) {
    ...
    const originalCollectionName = document.getElementById(
      "edit-product-collection"
    ).value;
    const title = document.getElementById("edit-product-name").value.trim();
    const newCollectionName = document.getElementById(
    ...
    const sizes = [];
    document
      .querySelectorAll("#editSizesContainer .size-item-m")
      .forEach((item) => {
        const value = item
          .querySelector('input[name="editSizeValue[]"]')
          .value.trim();
        if (value) {
          sizes.push(value);
        }
    ...
    let finalImageUrls = currentProduct.url.filter(
